...
export function HistoryPanel() {
  ...
  return (
    <aside className="w-80 h-full border-r border-border bg-card/30 backdrop-blur-sm flex flex-col">
      {/* 標題 */}
      ...
      {/* 歷史列表 */}
      <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent hover:scrollbar-thumb-muted">
          ...
          <div className="p-3 space-y-2">
            {filteredHistory.map((item) => (
              <div
                ...
                className="group w-full p-3 rounded-xl border border-transparent bg-transparent hover:bg-accent hover:border-border transition-all duration-200 text-left relative cursor-pointer"
              >
                {/* 內容 */}
                <div className="pr-16">
      ...
      <div className="p-4 border-t border-border space-y-2">
        <button 
          ...
          className="w-full py-2.5 px-4 rounded-xl border border-border bg-background hover:bg-accent text-sm text-foreground transition-colors duration-200 flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed"
        >
          <Trash2 className="w-4 h-4" />
          清空所有記錄
        </button>
